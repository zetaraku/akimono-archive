...
if ($ITEM->{flag}!~/s/) {		# s 陳列不可
	$disp.="<SPAN>※".$ITEM->{name}."は陳列しても売れません</SPAN><br>" if ($ITEM->{popular}==0);
	$disp.="<SPAN>※".$ITEM->{name}."は陳列してもほとんど売れません</SPAN><br>" if ($ITEM->{popular} > 800000);	}
...
if($DT->{item}[$itemno-1]>=$ITEM->{limit})
	...
	{$disp.='<BR>他店からモンスターを購入することはできません<BR>';}
else
{
