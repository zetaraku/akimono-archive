...
foreach my $cnt ($pagestart..$pageend)
{
	...
	if ($message =~ /\?code(\d+):(\d+)$/)
		{
		my $i;
		($message,$i)=split('\?code',$message);
		$detail=" > ".GetMenuTag('flag','[詳細]',"&class=$1&mode=$2&check=".crypt($1,$2));
		}
	...
	else
	{
		if($id==$DT->{id})
			...
			{$disp.="<small>".GetTime2FormatTime($tm)."</small> <SPAN>[情報]".$message."</SPAN>".$detail;}
		elsif($mode==3)
